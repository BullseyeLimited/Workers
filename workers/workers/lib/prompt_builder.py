...
def _card(thread, col):        # safe JSON→string
    return json.dumps(thread[col] or {}, ensure_ascii=False)
...
def make_block(thread_id, tier, limit=None):
    ...
    if limit:
        q = q.limit(limit)
    rows = q.execute().data
    lines = [x for r in rows for x in (r["narrative_summary"], r["abstract_summary"])]
    return "\n".join(lines)

def build_prompt(template, thread_id, raw_turns):
    tpl = open(f"/app/prompts/{template}.txt").read()
    thread = SB.table("threads").select("*").eq("id",thread_id).single().execute().data
    repl = {
